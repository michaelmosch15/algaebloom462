...
def preprocess(image):
    ...
    enhanced = cv2.equalizeHist(green_channel)  
    return enhanced
...
def kmeans_segmentation(image, k=3):
    green_weighted = image[:, :, 1] * 2  
    Z = np.dstack((image[:, :, 0], green_weighted, image[:, :, 2])).reshape((-1, 3))
    Z = np.float32(Z)
    criteria = (cv2.TERM_CRITERIA_EPS + cv2.TERM_CRITERIA_MAX_ITER, 10, 1.0)
    _, label, center = cv2.kmeans(Z, k, None, criteria, 10, cv2.KMEANS_RANDOM_CENTERS)
    label = label.flatten()
    centers_lab = cv2.cvtColor(center.astype(np.uint8).reshape(1, -1, 3), cv2.COLOR_BGR2LAB)[0]
    algae_cluster = np.argmax(centers_lab[:, 1])  # green focous
